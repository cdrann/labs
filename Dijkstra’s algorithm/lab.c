...
int main() {
	...
	int **matrix = create_matrix(num_vertices);
	matrix = fill_in_matrix(matrix, num_edges, num_vertices, input_file, output_file);

	node *vertexes = create_node(num_vertices);
	algo_dijkstra(num_vertices, matrix, vertexes, head - 1, tail - 1);

	print_all_distance(num_vertices, output_file, vertexes);
	print_path_head_to_tail(num_vertices, matrix, vertexes, head - 1, tail - 1, output_file);
...
// OUTPUT:
// single-source shortest paths ( am. = num vertices)
// shortest path(s) head->tail
