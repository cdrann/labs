...
int findMinInd(int *arr, int len) {
	...
	for (int i = 0; i < len; i++) {
		if (minInd == -1 && (arr[i] >= 0 || arr[i] == -2)
			|| arr[i] >= 0 && (arr[i] < arr[minInd] || arr[minInd] == -2))
		{
			minInd = i;
		}
	}
	...
	//for (int i = 0; i < len; i++) {
	//	if (minInd == -1 && arr[i] >= 0 || arr[i] >= 0 && arr[i] < arr[minInd])
	//		minInd = i;
	//}
	//if (minInd == -1)
	//	for (int i = 0; i < len; i++) {
...
void algo_dijkstra(graphNode *graph, int head, int num_vertices, int *black) {
	...
	while ((ind = findMinInd(gray, num_vertices)) >= 0) {
		...
		listTuple *temp = graph->neighbours->next;
		while (temp != NULL) {
...
void printLens(int *black, int n, FILE *output_file)
{
	for (int i = 0; i < n; i++)
	{
		...
		case -1:
			...
			break;
		case -2:
			fprintf(output_file, "(long long)INT_MAX+ ");
			break;
		default:
